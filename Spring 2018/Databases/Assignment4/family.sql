...
--
-- Host: localhost
-- Generation Time: Apr 12, 2018 at 11:05 PM
...
-- -- Procedures
-- --
-- CREATE DEFINER=`pricej60`@`localhost` PROCEDURE `printOldest`()
--     COMMENT 'during ties, one person is printed'
-- BEGIN
-- 		DECLARE oldest INT DEFAULT 0;
-- 		SELECT MAX(age) FROM Patron INTO oldest;
-- 		SELECT patron FROM Patron WHERE age = oldest;
-- 	END$$
...
--
-- Stand-in structure for view `getChildren`
--
CREATE TABLE IF NOT EXISTS `getChildren` (
`id` int(11)
,`bio` varchar(1000)
,`gender` varchar(1)
...
,`lastName` varchar(20)
,`lastKnownAddress` varchar(1000)
,`lastKnownPhoneNumber` varchar(12)
,`birth` date
,`birthLocation` varchar(50)
,`death` date
,`deathLocation` varchar(50)
,`parent1` int(11)
,`parent2` int(11)
);
...
--
-- Stand-in structure for view `getParents`
--
CREATE TABLE IF NOT EXISTS `getParents` (
`id` int(11)
,`bio` varchar(1000)
,`gender` varchar(1)
,`firstName` varchar(20)
...
--
-- Structure for view `getChildren`
--
DROP TABLE IF EXISTS `getChildren`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`pricej60`@`localhost` SQL SECURITY DEFINER VIEW `getChildren` AS select `Person`.`id` AS `id`,`Person`.`bio` AS `bio`,`Person`.`gender` AS `gender`,`Person`.`firstName` AS `firstName`,`Person`.`middleName` AS `middleName`,`Person`.`lastName` AS `lastName`,`Person`.`lastKnownAddress` AS `lastKnownAddress`,`Person`.`lastKnownPhoneNumber` AS `lastKnownPhoneNumber`,`Person`.`birth` AS `birth`,`Person`.`birthLocation` AS `birthLocation`,`Person`.`death` AS `death`,`Person`.`deathLocation` AS `deathLocation`,`Person`.`parent1` AS `parent1`,`Person`.`parent2` AS `parent2` from `Person` where ((`Person`.`parent1` = 1) or (`Person`.`parent2` = 1));

-- --------------------------------------------------------

--
-- Structure for view `getParents`
--
DROP TABLE IF EXISTS `getParents`;

-- CREATE ALGORITHM=UNDEFINED DEFINER=`pricej60`@`localhost` SQL SECURITY DEFINER VIEW `getParents` AS select `Person`.`id` AS `id`,`Person`.`bio` AS `bio`,`Person`.`gender` AS `gender`,`Person`.`firstName` AS `firstName`,`Person`.`middleName` AS `middleName`,`Person`.`lastName` AS `lastName`,`Person`.`lastKnownAddress` AS `lastKnownAddress`,`Person`.`lastKnownPhoneNumber` AS `lastKnownPhoneNumber`,`Person`.`birth` AS `birth`,`Person`.`birthLocation` AS `birthLocation`,`Person`.`death` AS `death`,`Person`.`deathLocation` AS `deathLocation`,`Person`.`parent1` AS `parent1`,`Person`.`parent2` AS `parent2` from `Person` where ((`Person`.`id` = (select `Person`.`parent1` from `Person` where (`Person`.`id` = 17))) or (`Person`.`id` = (select `Person`.`parent2` from `Person` where (`Person`.`id` = 17))));
...
CREATE VIEW getParents AS
SELECT * FROM Person
WHERE id = (SELECT parent1 FROM Person WHERE id = 17) or id = (SELECT parent2 FROM Person WHERE id = 17);

CREATE VIEW getChildren AS
SELECT * FROM Person
