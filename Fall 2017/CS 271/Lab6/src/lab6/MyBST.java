...
//need to finish this method, possibly add a helper method
        public int inBetween(T low, T high){
            int count = 0;
            MyNode<T> temp = root;
            int compareLow = temp.data.compareTo(low);
            int compareHigh = temp.data.compareTo(high);
            while(temp != null || compareHigh >= 0){
                if(compareLow > 0){
                    count++;
                }
                else{
                ...
                if(temp != null){
                    compareLow = temp.data.compareTo(low);
                    compareHigh = temp.data.compareTo(high);
                }
            }
            return count;
        }
